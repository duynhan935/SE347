...
export default function RestaurantList() {
    ...
    const handleToggleEnabled = async (res: Restaurant) => {
        ...
        const ok = await confirmAction({
            ...
            description: `Set status to ${newStatus ? "ENABLED" : "DISABLED"}?`,
            confirmText: "Confirm",
            cancelText: "Cancel",
        });
        if (!ok) return;
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="bg-white divide-y divide-gray-200">
                                ...
                                {paginatedRestaurants.map((res) => (
                                    <tr key={res.id}>
                                        <td className="px-6 py-4 whitespace-nowrap">{res.resName}</td>
                                        <td className="px-6 py-4 whitespace-nowrap truncate max-w-xs">{res.address}</td>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <button
                                                onClick={() => handleToggleEnabled(res)}
                                                className={`mr-4 ${
                                                    res.enabled
                                                        ? "text-red-600 hover:text-red-900 cursor-pointer"
                                                        : "text-green-600 hover:text-green-900 cursor-pointer"
                                                }`}
                                                title={res.enabled ? "Disable" : "Enable"}
                                            >
                                                {res.enabled ? (
