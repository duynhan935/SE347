    ...
    return (
        <div className="flex items-start gap-4 py-5 border-b border-gray-100 last:border-b-0">
            ...
            <div className="flex-grow min-w-0 space-y-1.5">
                ...
                {/* Category */}
                {item.categoryName && (
                    <p className="text-xs md:text-sm text-gray-500 font-medium">Category: {item.categoryName}</p>
                )}

                {/* Price per item - Small, light gray */}
                <p className="text-xs md:text-sm text-gray-400 font-medium">${item.price.toFixed(2)} each</p>

                {/* Size and Customizations - Small, light gray */}
                {item.sizeName && <p className="text-xs text-gray-400">Size: {item.sizeName}</p>}
                {item.customizations && (
                ...
                <p className="font-semibold text-sm text-gray-600 mt-2">${(item.price * item.quantity).toFixed(2)}</p>
            </div>
