...
export const authApi = {
    ...
    refreshAccessToken: async () => {
        // Backend reads refreshToken from HttpOnly cookie and returns new access token in `message`
        const response = await api.get<RefreshTokenResponse>("/users/refreshtoken");
        return response.data.message;
    },

    // Merchant approval requests (Admin)
    getMerchantRequests: async () => {
        const response = await api.get<User[]>("/users/merchants/consideration");
        return response.data;
    },
