...
export const blogApi = {
    ...
    updateBlog: async (blogId: string, blogData: BlogUpdateRequest): Promise<BlogDetailResponse> => {
        ...
        if (!blogData.featuredImage && (!blogData.images || blogData.images.length === 0)) {
            ...
            if (blogData.seo) {
                ...
                if (blogData.seo.metaDescription) formData.append("seo[metaDescription]", blogData.seo.metaDescription);
                if (blogData.seo.keywords && blogData.seo.keywords.length > 0) {
                    formData.append("seo[keywords]", blogData.seo.keywords.join(","));
                }
            }
    ...
    getComments: async (blogId: string, params?: { page?: number; limit?: number }): Promise<CommentListResponse> => {
        const queryParams = new URLSearchParams();
        if (params?.page) queryParams.append("page", params.page.toString());
        if (params?.limit) queryParams.append("limit", params.limit.toString());
    ...
    replyComment: async (
        ...
        const response = await api.post<CommentResponse>(`/blogs/${blogId}/comments/${commentId}/reply`, formData, {
            headers: {
                "Content-Type": "multipart/form-data",
            },
        });
